...
И правда, `std::forward` работает иногда даже лучше и выдаёт неопытному программисту более ожидаемые результаты, чем `std::move`. Однако сейчас речь идёт о смысле использования этих инструментов. `std::forward` требует явное указание шаблонного типа, что иногда захламляет код. К тому же явное назначение этой функции - идеальная передача, сохранение изначального типа объекта при <<перебрасывании>> его сквозь программу. Этих двух причин достаточно, чтобы использовать `std::forward` только по назначению, а при работе с взаимодействием объектов в случае надобности использовать `std::move`.
