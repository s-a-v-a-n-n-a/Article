...
Начинающий программист часто при работе с временными объектами копирует их и не задумывается о накладных расходах. Тем не менее, они есть. Думаю, многим известно, что в определённых случаях перемещение лучше копирования, но не всегда можно добиться перемещения везде, где хочется. Именно поэтому я решила написать статью с примерами и картинками о таких инструментах программирования, как std::move и std::forward.
...
Граф событий, иллюстрирующий рождение, передачу значения, вызовы функций и прочие события, я визуализировала при помощи пакета Graphviz. По этой причине приведу пояснение тому, что означают элементы на картинках:
<li>
...
<li>
Чёрные сплошные стрелки показывают связь между переменными и оператором, их связывающим. Например, если складываются переменные, то вы увидите такие стрелки, ведущие от слагаемых к результату;
</li>
<li>
Серый прямоугольный фон означает, что события происходят внутри функции, имя которой написано в самом верху этого прямоугольника;
</li>
<li>
Организация информации в прямоугольных узлах с примером указана ниже:
</li>
...
<!-- | Переданный аргумент | Результат</br> `std::move` | Результат</br> `std::forward` |
|:-------------------:|:---------:|:------------:|
| T | T&&</br> Объект перемещается | T&&</br> Объект перемещается |
| T& | T&&</br> Объект перемещается | __T&__</br> __Объект копируется__ |
| T&& | T&&</br> Объект перемещается | T&&</br> Объект перемещается | -->
